...
// Global tag: isolates the inards of the coe and automatically sends it over to run on the GPU
__global__
void
MonteCarlo( IN float* dbeforey, IN float* daftery, IN float* ddistx, OUT int* dsuccesses)
{
	...
	// Implied for loop, essentially broadcasting loop all over the GPU
	dsuccesses[gid] = 0;
...
main(int argc, char* argv[])
{
	...
	// setting up host version of the arrays
	{
	...
	// creating GPU version of the arrays in bits per float
	...
	// execute the kernel with set number of blocks and number of threads per block
	...
	// Adding up the number of successes
	{
